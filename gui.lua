...
--// CONFIG STORE
local store = DataStoreService:GetDataStore("YuriHub_" .. player.UserId)
...
--// SIDEBAR BUTTONS
local tabs = {"Main","Auto","Player","Webhook","Misc","Config"}
...
send.MouseButton1Click:Connect(function()
	pcall(function()
		HttpService:PostAsync(hookBox.Text,HttpService:JSONEncode({content=msgBox.Text}))
	end)
...
--// CONFIG
local export=Instance.new("TextBox")
export.Size=UDim2.new(0.9,0,0,60)
export.Position=UDim2.new(0.05,0,0,10)
export.Parent=Config

local import=Instance.new("TextBox")
import.Size=UDim2.new(0.9,0,0,60)
import.Position=UDim2.new(0.05,0,0,110)
import.Parent=Config

local function getConfig()
	return {tab=currentTab,scale=uiScale.Scale}
end

local function applyConfig(d)
	if d.tab then switch(d.tab) end
	if d.scale then uiScale.Scale=d.scale end
end

local expBtn=Instance.new("TextButton")
expBtn.Size=UDim2.new(0.9,0,0,25)
expBtn.Position=UDim2.new(0.05,0,0,75)
expBtn.Text="Export"
expBtn.Parent=Config

expBtn.MouseButton1Click:Connect(function()
	export.Text=HttpService:JSONEncode(getConfig())
	notify("Config exported")
end)

local impBtn=Instance.new("TextButton")
impBtn.Size=UDim2.new(0.9,0,0,25)
impBtn.Position=UDim2.new(0.05,0,0,175)
impBtn.Text="Import"
impBtn.Parent=Config

impBtn.MouseButton1Click:Connect(function()
	local ok,data=pcall(function()
		return HttpService:JSONDecode(import.Text)
	end)
	if ok then
		applyConfig(data)
